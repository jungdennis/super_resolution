...
# Argparse Setting
parser = argparse.ArgumentParser(description = "ThemISuRNet model을 이용해 Super Resolution을 진행합니다. (Train)")
...
if __name__ == "__main__":
    ...
    for i_epoch_raw in range(EPOCH):
        ...
        for batch, i_dataloader in enumerate(dataloader_train):
            ...
            loss_ssim = _ssim_train
            loss_cx = criterion_cx(i_batch_sr, i_batch_hr)

            _loss_train = 10 * loss_l1 + 10 * (1 - loss_ssim) + 0.1 * loss_cx
        ...
        for i_dataloader in dataloader_valid:
            ...
            with torch.no_grad():
                ...
                _loss_valid = 10 * loss_l1 + 10 * loss_ssim + 0.1 * loss_cx
