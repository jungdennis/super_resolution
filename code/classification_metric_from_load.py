...
path_rate = path_log + "/FAR_FRR_LR_SYSU_64.pt"
...
if __name__ == "__main__":
    metric_save_A = torch.load(path_log + "/metric_HR_A_Reg.pt")
    distance_same_A = metric_save_A['distance_same']
    distance_diff_A = metric_save_A['distance_diff']

    metric_save_B = torch.load(path_log + "/metric_HR_B_Reg.pt")
    ...
    metric_histogram(distance_same, distance_diff, title="Distribution of Distance (HR, Reg)", density = True)
    ...
    graph_FAR_FRR(threshold, FAR, FRR, show_EER = True, title = "Graph of FAR & FRR (HR, Reg)")
    graph_FAR_FRR(threshold, FAR, FRR, show_EER=True, log=False, title="Graph of FAR & FRR (HR, Reg)")
    print(f"EER : {EER}, threshold : {th}")
    torch.save({"EER" : EER, "th" : th}, path_log + "/EER_LR_SYSU_64.pt")